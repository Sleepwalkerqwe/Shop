...
const ManageOrders = () => {
  ...
  return (
    <div className="section__container p-6">
            ...
            orders.map((order, index) => (
              <tr key={index}>
                ...
                <td className="py-3 px-4 border-b flex items-center space-x-4">
                  ...
                  <button className="text-red-500 hover:underline" onClick={() => handleDeleteOder(order?._id)}>
                    Delete
                  </button>
                </td>
              </tr>
      ...
      {/* update order modal */}
      {selectedOrder && <UpdateOrderModal order={selectedOrder} isOpen={isModalOpen} onClose={handleCloseModal} />}
    </div>
  );
};
